...
# ---- Helper function to count logos ----
def get_logo_counts(results, class_names):
    if not results:
        return {}
    r = results[0]
...
def get_fps_and_length(cap):
    fps = cap.get(cv2.CAP_PROP_FPS)
    length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    return fps if fps > 0 else 25, length

def process_video_per_second_and_total(video_path, model, class_names, stframe):
    ...
    cap.release()
    # Calculate total logo frequencies
    total_logo_counts = {}
    if all_class_ids:
